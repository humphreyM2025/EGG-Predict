...
class EGGSignalProcessor:
    ...
    def bandpass_filter(self, signal, lowcut=0.015, highcut=0.2, order=3):
        ...
        # Order 3 bandpass butter filter requires at least 22 samples for default padlen (21)
        min_required = 22
        
        # If signal is shorter than 22 points, edge-pad it to prevent SciPy crash
        if len(signal) < min_required:
            pad_size = min_required - len(signal)
        ...
        # Safely compute padlen so it never exceeds array bounds
        padlen = min(21, len(signal) - 1)
    ...
    def extract_features(self, raw_signal):
        ...
        if len(raw_signal) == 0:
            raise ValueError("Provided EGG signal is empty.")

        filtered = self.bandpass_filter(raw_signal)
        
        # Ensure Welch nperseg is always valid for the signal length
        ...
        # Ensure distance parameter in find_peaks is valid
        peak_distance = max(1, int(self.fs * 15))
        peaks, _ = find_peaks(filtered, distance=peak_distance)
