...
# ROOT ROUTE: Always send visitors to the login page first
@app.route('/')
def index():
...
def delete_patient(patient_id):
    ...
    pred_ids = [pred.id for pred in patient.predictions]
    filenames = [pred.dataset_filename for pred in patient.predictions if pred.dataset_filename]

    if session.get('active_pred_id') in pred_ids:
        session.pop('active_pred_id', None)
    ...
    db.session.commit()
    flash(f'Patient {patient.full_name} and all their records were deleted.', 'success')
    return redirect(url_for('history'))
